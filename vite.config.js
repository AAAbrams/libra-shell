...
function resolveBareImportsFromModuleRoot() {
    ...
    return {
        ...
        async resolveId(source, importer) {
            const isBareImport = !source.startsWith('.') &&
                !source.startsWith('/') &&
...
function resolveCssEntry(cssEntry, projectRoot) {
...
function resolveDirectory(directory, projectRoot) {
...
function resolveOutputDir(outDir, projectRoot) {
...
function resolvePublicBasePath(publicOutDir, explicitBase) {
...
function parseInput(rawInput) {
    ...
    if (normalizedInput.startsWith('{')) {
        ...
        return Object.fromEntries(Object.entries(parsedInput).map(([key, file]) => [key, resolveFromRoot(file)]));
    }
